...
namespace UnityEditor
{
    ...
    internal class RuleTileEditor : Editor
    {
        ...
        public static string[] m_ModDirections = { "SW", "SE", "NW", "NE" };
        ...
        private float GetElementHeight(int index)
        {
            if (tile.m_TilingRules != null && tile.m_TilingRules.Count > 0)
            {
                switch (tile.m_TilingRules[index].m_Output)
                {
                    ...
                    case RuleTile.TilingRule.OutputSprite.Modulo:
                        return k_DefaultElementHeight + k_SingleLineHeight * (tile.m_TilingRules[index].m_Sprites.Length) + k_PaddingBetweenRules;
                }
            }
        ...
        internal static void RuleInspectorOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
        {
            ...
            if (tilingRule.m_Output == RuleTile.TilingRule.OutputSprite.Modulo)
            {
                if (tilingRule.m_Sprites.Length != 4)
                {
                    Array.Resize(ref tilingRule.m_Sprites, 4);
                }


                for (int i = 0; i < tilingRule.m_Sprites.Length; i++)
                {
                    GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), m_ModDirections[i]);
            ...
            else if (tilingRule.m_Output != RuleTile.TilingRule.OutputSprite.Single)
            {
