...
namespace UnityEngine
{
    ...
    public class RuleTile : TileBase
    {
        ...
        public class TilingRule
        {
            ...
            public enum OutputSprite { Single, Random, Animation, Modulo }
        }
                ...
                if (RuleMatches(rule, ref neighboringTiles, ref transform))
                {
                    switch (rule.m_Output)
                    {
                        ...
                        case TilingRule.OutputSprite.Animation:
                            ...
                            break;
                        case TilingRule.OutputSprite.Modulo:
                            int modIndex = Mathf.Abs((position.x % 2)) + Mathf.Abs(((position.y % 2) * 2));
                            tileData.sprite = rule.m_Sprites[modIndex];
                            break;
